...
void Scorpion::initialize_audio_effects() {
    engine::Audio * scorpion_attacking = nullptr; /**< Audio. Sound that represents when scorpion is attacking. */
    ...
    engine::Audio * scorpion_on_attack = nullptr; /**< Audio. Sound that represents when scorpion is on attack. */
...
void Scorpion::initialize_animations() {
    engine::Animation* walking_left_animation = nullptr; /**< Animation. Animation that represents scorpion's walk to the left. */
    ...
    engine::Animation* walking_right_animation = nullptr; /**< Animation. Animation that represents scorpion's walk to the right. */
    ...
    engine::Animation* idle_left_animation = nullptr; /**< Animation. Animation that represents scorpion's idle to the left. */
    ...
    engine::Animation* idle_right_animation = nullptr; /**< Animation. Animation that represents scorpion's idle to the right. */
    ...
    engine::Animation* attacking_left_animation = nullptr; /**< Animation. Animation that represents scorpion's attack to the left. */
    ...
    engine::Animation* attacking_right_animation = nullptr; /**< Animation. Animation that represents scorpion's attack to the right. */
    ...
    engine::Animation* on_attack_left_animation = nullptr; /**< Animation. Animation that represents scorpion's on attack to the left.  */
    ...
    engine::Animation* on_attack_right_animation = nullptr; /**< Animation. Animation that represents scorpion's on attack to the right. */
    ...
    engine::Animation* dying_left_animation = nullptr; /**< Animation. Animation that represents scorpion's death to the left. */
    ...
    /* Initialize booleans that references the scorpion's death animation process, to the left. */
    dying_left_animation->is_a_final_animation = true;
    dying_left_animation->in_loop = false;

    engine::Animation* dying_right_animation = nullptr; /**< Animation. Animation that represents scorpion's death to the right. */
    ...
    /* Initialize booleans that references the scorpion's death animation process, to the right. */
    dying_right_animation->is_a_final_animation = true;
    dying_right_animation->in_loop = false;

    /* Adds animation's components into a game object. */
...
engine::Animation* Scorpion::create_animation(
    ...
    std::string direction){
        ...
        engine::Game& game = engine::Game::get_instance(); /**< Game. Gets an instance of a game just initializated. */

        engine::Animation* animation = nullptr; /**< Animation. Can represents all functions about scorpion's animation. */
...
void Scorpion::initialize_as_physicable() {
    engine::Physics *physics = nullptr; /**< Physics. Gets a physics instance to scorpion of a game just initialized. */

    /* Changes physics of an object catched by an instance, turning it collidable. */
...
void Scorpion::initialize_hitboxes() {
    ...
    engine::Hitbox* scorpion_hitbox = nullptr; /**< Hitbox. Initialize scorpion's hitbox with its height. */
    ...
    engine::Hitbox* scorpion_attack = nullptr; /**< Hitbox. Initialize scorpion's hitbox when its attack. */
...
void Scorpion::on_event(GameEvent game_event) {
    ...
    /* If the scorpion gets an event to move to the left direction. */
    ...
    /* If the scorpion gets an event to move to the right direction. */
...
void Scorpion::notify(engine::Observable *game_object) {
    LittleGirl* little_girl = nullptr; /**< LittleGirl. Gets as observable that tells when little_girl is next. */
...
void Scorpion::attack() {
    ...
    std::string actual_x_state = ""; /**< String. Gets the actual state of the scorpion. */
...
void Scorpion::on_attack(GameObject *game_object) {
    ...
    if (is_alive()) {
        ...
        /* Condition that verifies if the scorpion is looking to the left. */
        if (actual_x_state == "LOOKING_LEFT") {
            /* Initiates the animation that show the scorpion being hitted left. */
            set_actual_animation(animations["on_attack_left_animation"]);
        }
        /* Condition that verifies if the scorpion is looking to the right. */
        else if (actual_x_state == "LOOKING_RIGHT") {
            /* Initiates the animation that show the scorpion being hitted right. */
...
void Scorpion::move(engine::GameObject* girl) {
    ...
    /* Gets X positions of the scorpion and of the girl and initializes they distance between them. */
    int distance_from_girl = 0;
    float scorpion_position = get_position_x();
    float girl_position = girl->get_position_x();

    /* Verifies the scorpion position on the X axis to define the scorpion's state. */
    if (scorpion_position > girl_position) {
        ...
        /* Verifies if the girl is in the fields vision of the scorpion. */
            ...
            /* Verifies if the girl is next or far away to the scorpion. */
            if (distance_from_girl >= 50) {
                ...
                /* Initiates the animation that show the scorpion walking to the left. */
                set_actual_animation(animations["walking_left_animation"]);
            }
            else {
                /* Verifies if the scorpion and the girl is in the same nivel. */
                if (same_nivel) {
        ...
        /* Verifies if the scorpion is to far away in front of scorpion. */
        if (distance_from_girl > 588) {
            ...
            if (distance_from_girl >= 200) {
                /* Updates the X position of the scorpion, making it walking. */
                set_position_x(get_position_x() + (same_nivel? 8 : 3));
                /* Initiates the animation that show the scorpion walking to the right. */
...
void Scorpion::on_collision(
    ...
    Platform* platform = nullptr; /**< Platform. References the map where is happening the collision. */
    platform = dynamic_cast<Platform *>(other);

    LittleGirl* little_girl = nullptr; /**< LittleGirl. References to LittleGirl object. */
    little_girl = dynamic_cast<LittleGirl *>(other);

    engine::Hitbox* my_hitbox = nullptr; /**< Hitbox. References to scorpion's hitbox. */
    my_hitbox = dynamic_cast<engine::Hitbox *>(p_my_hitbox);

    engine::Hitbox* other_hitbox = nullptr; /**< Hitbox. References to girl's hitbox. */
    other_hitbox = dynamic_cast<engine::Hitbox *>(p_other_hitbox);

    /* Verifies if the scorpion and the girl are on collision. */
    if (get_speed_y() >= 0 
        && platform 
    ...
    /* Verifies if the girl is attacking the scorpion. */
    if (little_girl 
        ...
        /* Verifies if the scorpion is under attack. */
        if (get_state("ACTION_STATE") == "ON_ATTACK") {
            return;
        }
        else {
