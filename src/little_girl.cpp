...
void LittleGirl::initialize_audio_effects() {
	...
	engine::Audio *little_girl_steps = new engine::Audio(
			...
			engine::Audio::CHUNK);
	const int little_girl_steps_duration = 1; /**< Integer. Default step sound 
		duration in seconds */
	little_girl_steps->set_duration(little_girl_steps_duration);
	const int little_girl_steps_volume = 45; /**< Integer. Default step sound 
		volume in seconds */
	little_girl_steps->set_effect_volume(little_girl_steps_volume);
	add_component(little_girl_steps);

	/* Initiates sound effect while the Little Girl is been hit */
	const int little_girl_hit_duration = 0.5; /**< Integer. Default hit sound 
		duration in seconds */
	engine::Audio *little_girl_getting_hit = new engine::Audio(
			"hit_me",
			"../assets/audios/effects_songs/menina_apanhando.wav",
			engine::Audio::CHUNK);
	little_girl_getting_hit->set_duration(little_girl_hit_duration);
	add_component(little_girl_getting_hit);

	/* Initiates sound effect while the Little Girl is using the sword */
	const int little_girl_sword_duration = 0.4; /**< Integer. Default sword sound 
		duration in seconds */
	engine::Audio *sword_song = new engine::Audio(
			"sword_attack",
			"../assets/audios/effects_songs/espada_fase_1.wav",
			engine::Audio::CHUNK);
	sword_song->set_duration(little_girl_sword_duration);
