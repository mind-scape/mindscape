...
namespace mindscape{
  ...
  class Scorpion : public Enemy {
    ...
    public:
      Scorpion(std::string p_name, std::pair<int, int> position, int p): Enemy(p_name, position, p){
        ...
        engine::Hitbox* scorpion_hitbox = new engine::Hitbox("scorpion_hitbox", this->get_position(), std::make_pair(40, 312), std::make_pair(180,8), game.get_renderer());
        engine::Hitbox* scorpion_attack = new engine::Hitbox("scorpion_attack", this->get_position(), std::make_pair(5, 200), std::make_pair(283,10), game.get_renderer());
        collidable = true;
