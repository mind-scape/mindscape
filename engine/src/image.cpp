...
void Image::draw(int x, int y) {
	SDL_Rect ret = {coordinatesOnTexture.first, coordinatesOnTexture.second, dimensionOnTexture.first,
	                dimensionOnTexture.second};
	SDL_Rect render_quad = {x + get_displacement().first, y + get_displacement().second, this->get_width(),
	                        this->get_height()};
	
	SDL_RenderCopy(renderer, texture, &ret, &render_quad);
}

void Image::set_values(std::pair<int, int> _dimensionOnScreen, std::pair<int, int> _dimensionOnTexture,
                       std::pair<int, int> _coordinatesOnTexture) {
