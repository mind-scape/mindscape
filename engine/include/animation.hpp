...
namespace engine{
  ...
  class Animation : public Image{
    ...
    protected:
      ...
      std::map<std::string, std::map<int,int> > sprites_order;

    public:

      void set_sprites_order(int);
      Animation(SDL_Renderer* p_renderer,
        ...
        actual_sprite(first_sprite), is_finished(false), use_default_image_size(p_use_default_image_size){ 
          time = new Timer(); 
          set_sprites_order(total_sprites);
        }
